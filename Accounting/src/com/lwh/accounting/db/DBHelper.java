...
public class DBHelper {
	...
	// 查询总收入
	public int getTotalInCome(int year,int month,int day,int type,int status,long flag){
...
//				" and month=" + month +" and day=" + day + " and type=" + type + " and status=" + status;
		...
		buffer.append("select sum(price) from ")
				.append(DB_TABLENAME)
				.append(" where 1=1 and type=")
				.append(type)
				.append(" and status=")
				.append(status)
				.append(" and year=").append(year);
		if(flag == 2){	//月
			buffer.append(" and month=").append(month);
		}else if(flag == 3 || flag == 0){	//0,3-日
			buffer.append(" and month=").append(month).append(" and day=").append(day);
		}
		
		System.out.println(buffer.toString());
		
		Cursor cursor = dbInstance.rawQuery(buffer.toString(), null);
		int totalPrice = 0;
		while (cursor.moveToNext()) {
